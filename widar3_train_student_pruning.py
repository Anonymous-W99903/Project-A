...
NOTE: based on v2.2, try structured pruning
"""
...
def apply_structured_pruning(model, amount=0.2, pruning_type='ln'):
    """
    Apply structured pruning to the model.
    :param model: PyTorch model to be pruned
    :param amount: pruning amount
    :param pruning_type: type of pruning ('ln' for structured pruning)
    """
...
def main() :
	...
	if args.cuda :
		...
		# torch.cuda.set_per_process_memory_fraction(0.4, args.gpu_id)
	...
	for param in baseline.parameters() :
		param.requires_grad = False
	# logging.info('Baseline param size = %fMB', count_parameters_in_MB(tnet))
	logging.info(
	...
	if args.kd_mode == 'logits' :
		...
		# criterionKD = st.MultiSoftTarget(args.T)
	...
	# Pruning and testing the best model
	logging.info('Applying structured pruning to the best model...')
	apply_structured_pruning(snet, amount=args.prune_amount, pruning_type='ln')
	
	# Save pruned model
	pruned_model_path = os.path.join(args.save_root, 'pruned_model.pth.tar')
	torch.save({
	   'epoch': best_epoch,
	   'net': snet.state_dict(),
	   'prec@1': best_top1,
	...
	logging.info(f'Pruned model saved to {pruned_model_path}')
	
	# Test pruned model
	logging.info('Testing the pruned model...')
	pruned_top1, pruned_top3, pruned_loss = test(test_loader, snet, criterions['criterionCls'])
	logging.info(f'Pruned Model - Loss: {pruned_loss:.4f}, Prec@1: {pruned_top1:.2f}, Prec@3: {pruned_top3:.2f}')

	logging.info(f'Pruned model test results - Prec@1: {pruned_top1:.2f}, Prec@3: {pruned_top3:.2f}')
	logging.info('')
	...
	for i, data in enumerate(zip(*train_loader_list), start=1) :
		...
		for x, target, domain in data :
			...
			if domain in args.train_domain:
				...
				with torch.no_grad():
					out_t = tnet(x)
				# calculate KD loss for domain-specific teacher
